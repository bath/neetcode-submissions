class Solution:
    def twoSum(self, numbers: List[int], target: int) -> List[int]:
        ...
        # switch solution to two pointers so we don't iterate on the whole list ?
        # how does the two pointers method actually save time though? its just O(n) ??
            # maybe because the array is already sorted we know if we have the 2nd item we need to skip some amount? 
                # ????
            # ok so its not a pure O(n) but a O(n * log n) which makes more sense

            for i in range(len(numbers)):
                # find the array we want to search
                    # we are already starting at i ?? wtf i guess
                l, r = i + 1, len(numbers) - 1
                tmp = target - numbers[i]
                while l <= r:
                    # run the scan until we converge on the same number
                    mid = l + (r - l) // 2
                    if numbers[mid] == tmp:
                        return [i+1, mid + 1]
